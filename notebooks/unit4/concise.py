...
def record_video(env, policy, out_directory, fps=30):
    ...
    while not terminate:
        ...
        action, _ = policy.act(state)
        state, reward, terminate, truncate, info = env.step(action)  # We directly put next_state = state for recording logic
        img = env.render()#mode="rgb_array")
        images.append(img)
    imageio.mimsave(out_directory, [np.array(img) for i, img in enumerate(images)], fps=fps)

record_video(eval_env, cartpole_policy, './replay.mp4')
